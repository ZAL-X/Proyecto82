...
export default class RideScreen extends Component {
    ...
    handleTransaction = async () => {
        ...
        if (!transactionType) {
            ...
            // Revisa la disponibilidad para iniciar el viaje 
            var isEligible = await this.checkUserEligibilityForRenting(userId);

            if (isEligible) {
                var { bikeType, userName } = this.state;
                this.assignBike(bikeId, userId, bikeType, userName);
                Alert.alert(
                    "Has rentado la bicicleta por 1 hora. ¡Disfruta tu viaje!"
                );
                this.setState({
                    bikeAssigned: true
                });
            ...
            // Revisa la disponibilidad para finalizar el viaje
            var isEligible = await this.checkUserEligibilityForReturning(userId);
        ...
        if (bikeRef.docs.length == 0) {
            ...
            bikeRef.docs.map(doc => {
                if (!doc.data().under_maintenance) {
                    //si la bicilceta está disponible entonces el tipo de transacción será "rented" de lo contrario será "regresar" 
    ...
    checkUserEligibilityForStartRide = async userId => {
        ...
        if (userRef.docs.length == 0) {
            ...
            // Revisa y actualiza el valor de elegibilidad del usuario
            // ya sea que deba ser 'true' o 'false'
            isUserEligible = true;
    ...
    checkUserEligibilityForEndRide = async (bikeId, userId) => {
        ...
        transactionRef.docs.map(doc => {
            ...
            // Revisa si 'lastBikeTransaction.userid' es igual a 'userId'. 
        ...
        // Devuelve la elegibilidad del estado del usuario 
        return                 ;
    };
    ...
    assignBike = async (bikeId, userId, bikeType, userName) => {
        ...
        //cambia el valor de la bicicleta asignada por el usuario
        db.collection("usuarios")
            .doc(userId)
            .update({
                bike_assigned: true
                ...
                <View style={styles.lowerContainer}>
                    ...
                    >
                        <Text style={styles.buttonText}>
                            {bikeAssigned ? "Finalizar el viaje" : "Desbloquear"}
                        </Text>
                    </TouchableOpacity>
                </View>
            </KeyboardAvoidingView>
